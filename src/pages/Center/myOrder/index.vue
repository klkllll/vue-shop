<template>
  <div class="order-right">
    <div class="order-content">
      ...
      <div class="orders">
        <table class="order-item" v-for="(order) in myOrder.data.records" :key="order.id">
          <thead>
            <tr>
              <th colspan="5">
          ...
          <tbody>
            <tr v-for="(orderDetail,index) in order.orderDetailList" :key="orderDetail.id">
              ...
              <td rowspan="2" width="8%" class="center" v-if="index == 0">小丽</td>
...
export default {
  ...
  methods:{
    async getData(){
        let {page, limit} = this
        this.myOrder =  await this.$API.reqOrderList(page, limit)
    },
    async gotoPageNo(page) {
        this.myOrder = await this.$API.reqOrderList(page, this.limit)
    },
  },
};
</script>
